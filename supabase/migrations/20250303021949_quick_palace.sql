...
-- Create a function to periodically clean expired invites
CREATE OR REPLACE FUNCTION clean_expired_invites_trigger()
RETURNS trigger AS $$
BEGIN
  -- Clean up expired invites when new ones are created
  PERFORM clean_expired_invites();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Add trigger to clean expired invites when new ones are created
CREATE TRIGGER clean_expired_invites_on_insert
AFTER INSERT ON league_invites
FOR EACH STATEMENT
EXECUTE FUNCTION clean_expired_invites_trigger();